...
export const getCampaign = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: campaign, error } = await supabase
      .from("campaigns")
      .select("id, org_id, assessment_slug, title, status, closes_at, launched_at, closed_at, created_at")
...
export const getCampaignReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: campaign, error } = await supabase
      .from("campaigns")
      .select("id, org_id, assessment_slug, title, status, organizations(individual_results_unlocked, min_aggregate_group_size)")
      .eq("id", data.campaignId)
      .maybeSingle();
    if (error || !campaign) return { error: "Campaign not found." };
    ...
    for (const r of rows) {
      if (r.primary_label) labelDistribution[r.primary_label] = (labelDistribution[r.primary_label] ?? 0) + 1;
    }
    ...
    let individuals: { email: string; primaryLabel: string | null; percentage: number | null }[] | undefined;
    if (unlocked) {
