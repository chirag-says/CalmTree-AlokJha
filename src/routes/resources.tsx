...
export const Route = createFileRoute("/resources")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Download practical psychology workbooks, journals and toolkits to reflect and grow." },
      { property: "og:title", content: `${SITE.name} Resources` },
      { property: "og:description", content: "Workbooks, journals and toolkits for practical self-work." },
    ],
  }),
  component: Page,
});
...
const items = [
  ...
  { t: "Emotional Intelligence Workbook", d: "Prompts and exercises to grow self-awareness and empathy." },
  { t: "Stress Management Toolkit", d: "Tools to reduce daily stress without overhauling your life." },
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {items.map((c) => (
            <article key={c.t} className="rounded-2xl border border-border bg-card p-6 flex flex-col hover:shadow-[var(--shadow-soft)] transition-shadow">
              ...
              <Button className="mt-5" variant="secondary"><Download className="h-4 w-4" />Download PDF</Button>
            </article>
          ))}
        </div>
      </section>
